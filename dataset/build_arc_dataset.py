...
## NOTE:
# # - they use random color augmentation, as well as flip/rotate/translate, etc., apparently with 1000 augmentations per example (puzzle)
...
def convert_single_arc_puzzle(
    results: dict, default_name: str, puzzle: dict, aug_count: int, dest_mapping: Dict[str, Tuple[str, str]]
):
    # Remove "name"
...
def load_puzzles_arcagi(results: dict, dataset_path: str, config: DataProcessConfig):
    ...
    test_examples_map = {"evaluation": [(1.0, ("test", "all"))], "_default": [(1.0, ("train", "all"))]}
...
def convert_dataset(config: DataProcessConfig):
    ...
    for split_name, split in data.items():
        ...
        for subset_name, subset in split.items():
            # Construct subset
            results = {
                k: [] for k in ["inputs", "labels", "puzzle_identifiers", "puzzle_indices", "group_indices"]
            }
                ...
                for puzzle in group:
                    ...
                    for _idx_ex, (inp, out) in enumerate(puzzle.examples):
                        inp, out = np_grid_to_seq_translational_augment(
                            inp, out, do_translation=enable_translational_augment and _idx_ex != no_aug_id
                        )

...
class HRMExperimentRunner:
    ...
    def create_config_file(self, base_config: str, overrides: Dict[str, Any], output_name: str) -> Path:
    ...
    def run_training(
        self, config_path: Path, experiment_name: str, use_distributed: bool = False, num_gpus: int = 1
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=3600 * 24 * 5,  # 5 day timeout
            )
            ...
            experiment_result = {
                "experiment_name": experiment_name,
                "error": "timeout",
                "runtime_seconds": 3600 * 24 * 5,
            }
            return experiment_result

        except Exception as e:
            print(f"❌ Experiment {experiment_name} failed with exception: {e}")
            experiment_result = {"experiment_name": experiment_name, "error": str(e)}
            return experiment_result
    ...
    experiments.append(
        {
            ...
            "overrides": {"data_path": "data/arc-test-minimal"},
        ...
        experiments.append(
            {
                ...
                "overrides": {"data_path": f"data/arc-aug-{aug}", "run_name": f"augmentation_{aug}"},
...
def main():
    ...
    for i, exp in enumerate(filtered_experiments):
        ...
        # Create config file
        config_path = runner.create_config_file(exp["base_config"], exp["overrides"], exp["name"])

        # Run experiment
        result = runner.run_training(
            config_path, exp["name"], use_distributed=use_distributed, num_gpus=num_gpus
        )

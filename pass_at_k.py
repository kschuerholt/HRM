...
def compute_pass_at_k_from_preds(
    ...
    for identifier, input_tensor, label_tensor in zip(
        filtered_preds["puzzle_identifiers"], 
        filtered_preds["inputs"], 
    ...
    for i, (identifier, input_tensor, pred) in enumerate(zip(
        filtered_preds["puzzle_identifiers"],
        filtered_preds["inputs"], 
        preds
    )):
    ...
    try:
        ...
        with torch.inference_mode():
            ...
            if all_preds:
                ...
                if rank == 0:
                    pass_at_k_results = compute_pass_at_k_from_preds(
                        all_preds, 
                        identifier_map, 
                        k_values
                    )
